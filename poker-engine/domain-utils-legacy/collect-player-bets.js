...
exports = module.exports = function takeBet(gs, fromIndex) {
  ...
  return eachFrom(gs.players, fromIndex, function(player) {
    ...
    if (player.status == playerStatus.active && !player[Symbol.for('is-all-in')]){
      ...
      if (player.chipsBet < gs.callAmount || gs.players.filter(x => x.id != player.id && x.status == playerStatus.active && !x[isAllin]).length > 0){
